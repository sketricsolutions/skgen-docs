...
- A Teamspace Runtime API key beginning with `sk_runtime_`

Create the key from **Teamspace settings → API keys → Create key → Runtime key**. Admin API keys cannot run agents through this SDK.
