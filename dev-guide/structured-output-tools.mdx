...
Structured Output is a model-callable tool that emits schema-validated JSON during an agent run. It is useful when your application needs to render a card, save typed project state, trigger routing, or pass a predictable payload to downstream code while the agent remains conversational.

<Note>
Structured Output does not pause the run. Use Human Input to collect a person's answer, then let the agent call Structured Output after the required values are available.
</Note>
...
```json
{
  "id": "tool-node-recommendations",
  "type": "toolNode",
  "data": {
    "name": "emit_render_recommendations_ndations",
...
`TOOL_CALL_START.tool_call_arguments` contains the model-supplied object. `TOOL_CALL_END.tool_call_result` contains the validated compact JSON result. Treat the end event as confirmation that runtime validation succeeded.
