...
Every Runtime API request requires a Teamspace Runtime API key beginning with `sk_runtime_`.

Create it from **Teamspace settings → API keys → Create key → Runtime key**. Choose the narrowest project and agent scope that supports the integration. The plaintext key is shown once.

**Required Header:**

```http
API-KEY: sk_runtime_YOUR_KEY
```
...
```bash
export SKETRICGEN_RUNTIME_API_KEY="sk_runtime_YOUR_KEY"
...
| -------------- | ------ | -------- | --------------------------- |
| `API-KEY`      | string | Yes      | Teamspace Runtime API key beginning with `sk_runtime_` |
