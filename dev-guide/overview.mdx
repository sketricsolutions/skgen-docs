...
| **Python SDK** | You want one typed client for runtime and supported management operations | API key with **Run agents**, **Manage resources**, or both | Agent execution, HITL, streaming, file uploads, and supported Teamspace resource management |
| **Node.js SDK** | You want a typed JavaScript client for the Runtime API | API key with **Run agents** access, beginning with `sk_api_` | Agent execution, streaming, and supported file uploads |
...
- Use the unified [Python SDK](/dev-guide/python-sdk) for runtime and supported management operations, or the [Node.js SDK](/dev-guide/node-sdk) for a higher-level Runtime API client.
