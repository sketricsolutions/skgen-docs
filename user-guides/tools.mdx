---
title: "Tools"
description: "Connect 2000+ apps, knowledge bases, web search, code interpreter, image generation, REST APIs, and MCP servers as tools for your AI agents."
---
...
They can be attached directly to an agent node or added independently as **Tool Nodes** on the AgentSpace.
...
Scanned PDFs are supported — the backend extracts and parses text for indexing into the knowledge base.
...
- Generating or editing PDFs, CSVs, or charts (currently **beta** — generated files may not yet be surfaced cleanly in the product UI).
...
- Name and description
- MCP Server URL
- Connection type (Streamable HTTP or SSE over HTTP)
...
- Use Image Generation when your workflow needs visual outputs—marketing content, product mockups, or social media graphics. Multi-step asset workflows (generate → save → upload to another app) are still experimental.
- Keep credentials private and rotate API keys periodically.
- **Free plan note:** Marketplace app integrations on the free plan are **non-persistent** — when the session ends or the tab is closed, connected apps disconnect. Built-in tools (web search, etc.) remain available but are credit-based.
