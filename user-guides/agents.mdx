---
title: "Agents"
description: "Configure AI agents in SketricGen — set instructions, select models, attach tools, define structured I/O, and use Agent-as-a-Tool for modular workflows."
keywords: ["agents", "instructions", "tools", "handoffs", "multi-agent"]
---
...
Agents are configured directly on the AgentSpace. Every agent you add defines a new unit of reasoning or execution. The clearer the agent’s role and scope, the more predictable and interpretable the overall workflow becomes.
...
Each agent runs on a GPT model provided by SketricGen. Supported versions include **GPT-4o**, **GPT-4o Mini**, **GPT-4.1**, **GPT-4.1 Mini**, and **GPT-5.2**.

Different models vary in speed, reasoning depth, and credit cost.

**Guideline:**

Choose lighter models like GPT-4o Mini for fast responses and heavier models such as GPT-4.1 or GPT-5.2 for agents that perform analytical or multi-step reasoning.

![Screenshot showing model selection interface with supported GPT models](/images/user-guides/agent/agent_models.png)
...
- API Request (connect to REST endpoints)
...
Configure handoff conditions on each edge so the agent routes to the right specialist based on intent (e.g. "handoff if user asks about X").
