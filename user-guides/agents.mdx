...
When an application needs predictable JSON, connect the dedicated [Structured Output tool](/dev-guide/structured-output-tools) to the agent that should emit it. Structured Output is a tool, not an agent-level response-schema setting.

## Sub-Agents

<span id="agent-as-a-tool"></span>

Older interfaces described a specialist agent as an **agent as a tool**. In the current product, add it as a **sub-agent** in Agent Build. The main agent delegates a bounded task to the sub-agent, receives its result, and remains responsible for the user conversation and final answer.

Sub-agents cannot contain their own sub-agents. Give each one a distinct responsibility, focused instructions, and only the tools it needs. See [Agent Build](/ai-workforce/agent-build#main-agents-and-sub-agents) for the current configuration surface.

## Related

- [Brand Agents Overview](/brand-agents/overview)
- [Agent Space and Agent Build](/ai-workforce/agent-spaces)
